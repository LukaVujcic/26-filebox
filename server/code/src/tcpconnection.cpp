...
QMap<QTcpSocket *, QString> TCPConnection::users_map =
...
void TCPConnection::quit() {
    ...
    foreach (QTcpSocket *socket, sockets) { socket->close(); }
...
void display_files_in_folder(
    QString current_path, QString original_path, Zipper &zip, QString user) {
    QDir dir(current_path);

    if (dir.isEmpty()) {
        char *dir_path =
            (dir.canonicalPath().right(
                 (dir.canonicalPath().size() - original_path.size())) +
                "/")
                .toLocal8Bit()
                .data();
    ...
    foreach (
        auto name, dir.entryInfoList(QDir::AllEntries | QDir::NoDotAndDotDot,
                       QDir::DirsFirst)) {
        if (name.isDir()) {
            ...
            char *file_path =
                name.filePath()
                    .right(name.filePath().size() - original_path.size())
                    .toLocal8Bit()
                    .data();
            file_path =
                (user + static_cast<QString>(file_path)).toLocal8Bit().data();
            zip.add(file_path, zip.SaveHierarchy);
        }
    }
}

void get_files_and_folders(
    QString current_path, QString &server_path, Zipper &zip) {
...
void TCPConnection::readyRead() {
    auto *socket = static_cast<QTcpSocket *>(sender());
    QString userFolder =
        QDir(USERS_FOLDER + TCPConnection::users_map[socket]).absolutePath();
    ...
    if (is_download_request(REQUEST)) {
        ...
        //        "/home/luka/Desktop/FileBox-repo/26-filebox/server/users/admin";

        //        Zipper
        //        zipper("/home/luka/Desktop/FileBox-repo/26-filebox/server/users/ziptest.zip");
        //        display_files_in_folder(user_path, user_path, zipper,
        //        "filesystem"); zipper.close();
        //        sendFile("/home/luka/Desktop/FileBox-repo/26-filebox/server/users/ziptest.zip");
        //        QFile
        //        zip("/home/luka/Desktop/FileBox-repo/26-filebox/server/users/ziptest.zip");
        //        zip.remove();
        /*TODO username*/
        ...
        display_files_in_folder(
            dir.absolutePath(), dir.absolutePath(), zipper, "filesystem");
        zipper.close();
        sendFile(ZIPPER_LOCATION);
        QFile zip(ZIPPER_LOCATION);
        zip.remove();
        ...
        std::string new_folder =
            (socket->readLine(1000)).trimmed().toStdString();
        ...
        if (!fs::exists(folder_path)) {
            ...
            while (start <= ULONG_MAX) {
                std::string tmp_folder{
                    new_folder + " (" + std::to_string(start) + ")"};
        ...
        QString destination_path =
            (userFolder + socket->readLine(1000)).trimmed();
        ...
        try {
            // delete izdvojiti u posebnu metodu koja ce se pozivati ovde i u
            // paste funkciji
...
bool TCPConnection::checkProfile(
    const QString &username, const QString &password, QFile &file) {
    file.open(QIODevice::ReadWrite | QIODevice::Text);
    while (!file.atEnd()) {
...
QTcpSocket *TCPConnection::createSocket() {
    ...
    connect(
        socket, &QTcpSocket::disconnected, this, &TCPConnection::disconnected);
    connect(socket, &QTcpSocket::readyRead, this, &TCPConnection::readyRead);
    connect(
        socket, &QTcpSocket::bytesWritten, this, &TCPConnection::bytesWritten);
    connect(
        socket, &QTcpSocket::stateChanged, this, &TCPConnection::stateChanged);
    // connect(socket, static_cast<void
    // (QTcpSocket::*)(QAbstractSocket::SocketError)>(&QTcpSocket::error), this,
    // &TCPConnection::error);
