...
TCPServer::~TCPServer() { qDebug() << this << "destroyed"; }
...
void TCPServer::startRunnable(TCPRunnable *runnable) {
    ...
    connect(this, &TCPServer::closing, runnable, &TCPRunnable::closing,
        Qt::QueuedConnection);
    connect(runnable, &TCPRunnable::started, this, &TCPServer::started,
        Qt::QueuedConnection);
    connect(runnable, &TCPRunnable::finished, this, &TCPServer::finished,
        Qt::QueuedConnection);
    connect(this, &TCPServer::connecting, runnable, &TCPRunnable::connecting,
        Qt::QueuedConnection);
    connect(this, &TCPServer::idle, runnable, &TCPRunnable::idle,
        Qt::QueuedConnection);
...
void TCPServer::acceptSingle(qintptr handle) {
    ...
    if (!runnable) {
        qCritical() << this << "could not find runnable to accept connection: "
                    << handle;
        return;
    }
...
void TCPServer::startThreaded() {
    qDebug() << this
             << "MODE_THREADED - runnables will be created as connections are "
                "accepted";
...
void TCPServer::accept(qintptr handle, TCPRunnable *runnable) {
    ...
    if (!connection) {
        qCritical() << this
                    << "could not find connection to accept connection: "
                    << handle;
        return;
    }
