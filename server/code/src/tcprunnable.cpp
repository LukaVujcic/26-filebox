...
void TCPRunnable::connecting(
    qintptr handle, TCPRunnable *runnable, TCPConnection *connection) {
    if (runnable != this)
        return;

    qDebug() << this << "Connecting: " << handle << " on " << runnable
             << " with " << connection;
...
void TCPRunnable::idle(int value) {
    foreach (TCPConnection *connection, connections) {
        ...
        qDebug() << this << connection << " idle for " << idle << " timeout is "
                 << value;
...
void TCPRunnable::addSignals(TCPConnection *connection) {
    ...
    connect(connection, &TCPConnection::opened, this, &TCPRunnable::opened,
        Qt::QueuedConnection);
    connect(connection, &TCPConnection::closed, this, &TCPRunnable::closed,
        Qt::QueuedConnection);
    connect(this, &TCPRunnable::quit, connection, &TCPConnection::quit,
        Qt::QueuedConnection);
}
